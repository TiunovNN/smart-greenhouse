...
BOM = [
    ...
    (
        ...
        "Рекомендуемый хост HA; док. §1.1; оригинальная плата",
        14000,
        "iarduino.ru",
    ),
    (
        ...
        "PoE + слот NVMe; док. §1.1; оригинал Waveshare",
        4000,
        "onpad.ru",
    ),
    (
        ...
        "Загрузочный диск для HA OS; док. §1.1; форм‑фактор 2230 редкий — проверить 2230 перед покупкой (не 2280)",
        2400,
        "Ozon",
    ),
    (
        ...
        "Охлаждение в сетевом шкафу; док. §1.1; официальный Active Cooler (~841 ₽) + алюминиевый корпус (~859 ₽, onpad.ru)",
        2000,
    ...
    (
        ...
        "Уже есть; Pi HA к PoE‑коммутатору; док. §1.1 — кол‑во 0 к заказу",
        0,
        "",
    ),
    ...
    (
        ...
        "Уже есть; док. §1.2 — кол‑во 0 к заказу",
        0,
        "",
    ),
    (
        ...
        "Уже есть; наружная PoE AP ~5 м от теплицы; док. §1.2/§3",
        0,
        "",
    ),
    (
        ...
        "Уже есть; питает Pi HA + Stellar 6; док. §1.2",
        0,
        "",
    ),
    (
        ...
        "Уже есть; ~15–20 м наружный Cat6 к mesh AP; док. §1.2/§3 — кол‑во 0 к заказу",
        0,
        "",
    ),
    ...
    (
        ...
        "greenhouse-watering + greenhouse-climate; док. §1.3; разъём U.FL для внешней антенны",
        1000,
        "Youbot",
    ),
    (
        ...
        "Внешние антенны вне металлического корпуса; док. §1.3; реселлер AliExpress — доставка 2–4 нед. vs местный склад",
        450,
    ...
    (
        ...
        "Питание логики: ESP32, датчики, PCA9685 VCC; док. §1.3/§2.5; оригинал Mean Well",
        2100,
        "meanwellrus.ru",
    ),
    (
        ...
        "Питание реле и соленоидов; док. §1.3/§2.5; оригинал Mean Well",
        2850,
        "meanwellrus.ru",
    ),
    (
        ...
        "По одному на ESP32 полива (клапаны полива и бака); док. §1.3/§1.5; SRD-12VDC-SL-C + PC817",
        350,
        "CompactTool",
    ),
    (
        ...
        "Драйвер MG996R на greenhouse-climate; док. §1.3/§2.4",
        650,
        "Amperkot",
    ),
    (
        ...
        "Уже в наличии по док.; по одному на катушку реле/соленоида; док. §2.1",
        0,
        "",
    ),
    (
        ...
        "Полевой щит у теплицы; док. §1.3; DKC R5ST0342 с монтажной платой",
        5500,
        "Electroprice",
    ),
    (
        ...
        "1 комплект; WAGO 221 + сальники DKC PG + DIN‑рейка; док. §1.3; оценка набора",
        2500,
        "Chipdip + IEK",
    ),
    (
        ...
        "Линии датчиков внутрь теплицы; док. §1.3/§2.5; Extralink EX.32341 или аналог DKC outdoor FTP",
        2800,
        "Ozon",
    ),
    (
        ...
        "Опционально; удерживает клапаны в безопасном состоянии при отключении питания; док. §1.3/§6.4; класс WGP V1203",
        3000,
        "Ozon",
    ),
    (
        ...
        "Антиконденсат в корпусе; док. §1.3/§5.1; вентиляционный винт IP67 (аналог Gore vent)",
        700,
        "Ozon",
    ),
    (
        ...
        "Оценка; док. §2.5/§6.4; IEK/DEK 1P 10 A",
        400,
        "Chipdip",
    ),
    (
        ...
        "Отдельно от LRS-50-5; док. §2.4/§5.2; закрытый БП DONE 5 V 3 A 15 W",
        1000,
        "Chipdip",
    ),
    (
        ...
        "Уже есть; подтяжки DS18B20 + I2C; док. §2.1 — кол‑во 0 к заказу",
        0,
        "",
    ),
    (
        ...
        "Уже есть; опциональный debounce поплавка; док. §5.2 — кол‑во 0 к заказу",
        0,
        "",
    ),
    # 1.4 Датчики
    (
        ...
        "Центр, север, юг; I2C; док. §1.4/§2.3",
        350,
        "Youbot",
    ),
    (
        ...
        "Потолок + уровень растений; док. §1.4",
        200,
        "Youbot",
    ),
    (
        ...
        "Бак + линия полива; OneWire; док. §1.4/§2.2",
        200,
        "YourDuino",
    ),
    (
        ...
        "Верхний уровень бака; док. §1.4/§2.2; бытовой поплавок (не промышленный ПДУ)",
        350,
        "Ozon",
    ),
    (
        ...
        "Учёт полива и наполнения бака; док. §1.4",
        450,
        "Youbot",
    ),
    ...
    (
        ...
        "Полив + наполнение бака; fail-safe NC; док. §1.5; латунь 12 V DC прямого действия; реселлер AliExpress — проверить NC и мин. давление 0 bar",
        900,
        "AliExpress (alishops.ru)",
    ),
    (
        ...
        "Привод форточек через PCA9685; док. §1.5/§2.4; MG996R 180° с металлическими шестернями",
        450,
        "Youbot",
    ),
    # 1.6 Монтаж и запас
    (
        ...
        "Док. §1.6 сбалансированная строка бюджета (~10% запас); уменьшено после экономии на компонентах",
        3500,
        "Chipdip + Ozon",
    ),
]
...
def write_xlsx(rows):
    ...
    meta["A3"] = "Источник"
    meta["B3"] = "docs/smart-greenhouse-design.md (разделы 1.1–1.6)"
