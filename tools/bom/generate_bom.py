...
BOM = [
    ...
    (
        ...
        "Драйвер текущего MG996R/servo-варианта на greenhouse-climate; для линейных актуаторов не нужен; док. 04 §1.3/04 §2.4",
        650,
        "Amperkot",
    ),
    (
    ...
    (
        "Щит IP65 у теплицы",
        "Fuse + TVS protection kit for 12 V loads",
        1,
        800,
        "RUB",
        "https://www.chipdip.ru/search?searchtext=предохранитель+TVS+12V",
        "Предохранитель на силовую линию, TVS/защита моторов и клеммы; док. 04 §2.1; не экономить на силовой части",
        1200,
        "Chipdip",
    ),
    (
    ...
    (
        ...
        "Только если оставлять MG996R; отдельно от LRS-50-5; док. 04 §2.4/01 §5.2; закрытый БП DONE 5 V 3 A 15 W",
        1000,
        "Chipdip",
    ),
    (
        ...
        "Подтяжки DS18B20 + I2C ADS1115 на ESP32 №1 (GPIO21/22); doc. 04 §2.1",
        0,
        "Chipdip",
    ),
    (
        "Щит IP65 у теплицы",
        "Capacitor 100 nF (level contact debounce, optional)",
        0,
        0,
        "RUB",
        "https://www.chipdip.ru/search?searchtext=конденсатор+100нФ",
        "Уже есть; опциональный аппаратный debounce контактного уровня; док. 01 §5.2 — кол‑во 0 к заказу",
        0,
        "",
    ),
    # 1.4 Датчики
    (
        "Датчики",
        "AHT20/AHT21 temperature/humidity module (budget option)",
        0,
        180,
        "RUB",
        "https://www.ozon.ru/search/?text=AHT20+I2C+датчик+влажности",
        "Бюджетная альтернатива SHT31; ставить в вентилируемый экран, не в прямой конденсат; док. 03 §1.4",
        300,
        "Ozon / AliExpress",
    ),
    (
    ...
    (
        "Датчики",
        "Capacitive Soil Moisture Sensor v1.2/v2.0 (budget + spares)",
        0,
        160,
        "RUB",
        "https://www.ozon.ru/search/?text=Capacitive+Soil+Moisture+Sensor+v1.2",
        "Бюджетная замена VH400: покупать 6+2, герметизировать верх платы, калибровать каждый зонд; не резистивные YL-69/FC-28; док. 03 §1.4.3",
        250,
        "Ozon / AliExpress",
    ),
    (
        "Датчики",
        "Sealed capacitive soil moisture probe analog IP68 (Vegetronix VH400 class)",
        6,
        900,
        "RUB",
        "https://www.ozon.ru/search/?text=Vegetronix+VH400+soil+moisture",
        "П‑layout: 3 зонда на плечо L/R; корень 10–15 cm; сезон в мокрой почве; не резистивные модули; doc. 03 §1.4.3 / 04 §2.2",
        1200,
        "Ozon / AliExpress",
    ),
    (
        ...
        220,
        "RUB",
        "https://www.youbot.ru/search?q=ADS1115",
        "ESP32 watering: ADDR 0x48 + 0x49; 6 analog soil channels; GPIO21/22; doc. 04 §2.2",
        280,
        "Youbot",
    ),
    (
        "Датчики",
        "Stainless tank level contacts kit (3 bolts + ring terminals)",
        1,
        200,
        "RUB",
        "https://www.ozon.ru/search/?text=болт+нержавеющий+M4+клемма+кольцевая",
        "Три контакта в баке: верхний, средний, нижний общий; док. 03 §1.4 / 04 §2.2",
        350,
        "Ozon",
    ),
    (
    ...
    (
        "Исполнительные устройства",
        "Linear actuator 12 V with limit switches (IP54/IP65)",
        2,
        2500,
        "RUB",
        "https://www.ozon.ru/search/?text=линейный+актуатор+12V+100mm+концевики+IP65",
        "Рекомендуемый привод форточек для реальной эксплуатации; нужен H-bridge/реле реверса и обновление ESPHome; док. 03 §1.5 / 04 §2.4",
        3500,
        "Ozon / AliExpress",
    ),
    (
        "Исполнительные устройства",
        "MG996R servo (metal gears, prototype/light vents only)",
        2,
        320,
        "RUB",
        "https://www.youbot.ru/product/servoprivod-mg996r-180-gradusov-metall",
        "Текущая ESPHome/PCA9685-схема; только для стенда или лёгких створок, не лучший полевой вариант; док. 03 §1.5 / 04 §2.4",
        450,
        "Youbot",
    ),
    (
