...
name: home-assistant
description: >-
  Home Assistant YAML configuration and automation best practices. Covers automations, helpers,
  Jinja2 templates, notification patterns, and ESPHome integration patterns. Use when editing
  homeassistant/ configs, writing automations, or reviewing HA YAML in the smart-greenhouse project.
---

> Adapted from [aurora-smart-home/home-assistant](https://github.com/tonylofgren/aurora-smart-home/tree/main/home-assistant).
> Bundled references live in `references/`; upstream may add `*-2026-official.md` files not included here.
...
| YAML element | Reference (bundled) |
|---|---|
| Actions (`action:`, `target:`, `data:`) | `references/actions.md` |
| Triggers (`triggers:`, `trigger:`) | `references/triggers-advanced.md` |
| Conditions (`conditions:`, `condition:`) | `references/conditions.md` |
| Automations (structure, modes) | `references/automations.md` |
...
3. **Target path?** For smart-greenhouse: automations → `homeassistant/automations/`, helpers → `homeassistant/`.

4. **HA Version?** (for deprecated syntax awareness; use modern plural keys and `action:` not `service:`)
