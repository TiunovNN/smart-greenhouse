...
> Adapted from [aurora-smart-home/esphome](https://github.com/tonylofgren/aurora-smart-home/tree/main/esphome).
> For this repo, verify GPIO and entity naming against `docs/04-esp32-and-cabinet.md` before editing firmware.
...
Board confirmed? ──no──▶ Ask again
    │ yes
    ▼
Battery/actuator/outdoor/>5V? ──yes──▶ Perform hardware safety review
    │ no (or safety reviewed)              │ blocks if critical risk found
    ▼                                      ▼
Ask: Output method?              ◀── safety cleared
    │
    ▼
deep_sleep / battery / solar / power bank? ──yes──▶ Calculate power budget
    │ no (or after budget)
    ▼
...
- [ ] Hardware safety review completed for battery/actuator/outdoor/>5V projects
...
### Power
- [ ] Power budget calculated if project uses deep_sleep, battery, solar, or power bank
