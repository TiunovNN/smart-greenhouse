...
Три датчика SHT31 дают картину **в разных точках объёма** теплицы (типичный размер 3×4 … 3×6 м). Вход — со стороны двери/форточек; «противоположная сторона» — дальний торец. Центр наверху — под потолком/коньком, рядом с BH1750 «потолок».
...
**Монтаж:** корпус SHT31 вдали от прямого солнеца и капель конденсата; кабель FTP (SDA/SCL/5 V/GND) в щит. На каждой шине I2C — **разные** адреса 0x44 и 0x45 (перемычка ADDR на модуле GY‑SHT31‑D).
