...
Третье вычислительное устройство в щите — **не ESP32**: одноплатник для **локального** RTSP‑захвата с PoE‑камер, опционального ONNX pre-filter и **передачи снимков на Pi**. **Без доступа в Internet**; **без** ключей Yandex Cloud на устройстве. Подробнее — [05-computer-vision.md §3.4](05-computer-vision.md#34-edge-sbc-в-щите-ip65).
...
*SBC — локальный edge capture only; Yandex upload/analyze на Pi. Optional BOM CV ([05 §11](05-computer-vision.md#11-bom-дополнение)), не в базовые ~52 000 ₽.*

### 1.5. 3D‑печать: крепление плат на DIN‑рейку (STL)

В щите DKC R5ST0342 (или аналог) контроллеры удобно разместить на **стандартной 35 mm DIN‑рейке** (EN 60715 / ГОСТ Р МЭК 60715). Заводских корпусов под **ESP32‑DevKitC‑32U** и **Radxa ZERO 3W** с DIN‑креплением почти нет — типичное решение: **напечатать** корпус или монтажную пластину.
...
**Рекомендуемые модели (проверено по каталогам Printables, Thingiverse, MakerWorld, Cults3D, GitHub; июнь 2026):**
...
**Ближайшие аналоги Radxa ZERO 3W:** габарит **совпадает с Raspberry Pi Zero / Zero 2 W** (65 × 30 mm, те же M2.5). **Orange Pi Zero 2W** (52 × 46 mm) — **другой** форм‑фактор, не подходит без переделки.

**Русскоязычные источники:** на [3dtoday.ru](https://3dtoday.ru) есть обсуждения DIN‑креплений и отдельные клипсы для самодельных щитов; готовых STL «Radxa / DevKitC + DIN» мало — проще брать модели выше.

**Практика для IP65‑щита:** печатать из **PETG** (или ABS); 3–4 периметра, infill 20–30 %; внутри уже стоят Gore vent и силика‑гель ([§1.3](#13-щит-ip65-снаружи-у-входа--контроллеры-и-питание)). Для ESP32 с **U.FL** — вырез или отверстие под pigtail; GPIO и FTP — через нижние/боковые окна корпуса или открытые кронштейны.
...
1. **Разделение питания:** БП 5 В (ESP32, датчики, PCA9685 логика при servo‑варианте) и БП 12 В (реле, соленоиды, линейные актуаторы) — отдельные Mean Well; общая только «земля» (GND) в одной точке (звезда).
...
3. **Flyback / surge protection:** диод **1N4007** параллельно каждой катушке (реле/сolenoid): катод к «+» 12 В, анод к выходу реле. Для моторов/актуаторов — драйвер с встроенной защитой или TVS на 12 V, отдельный предохранитель и проверка тока заклинивания.
...
6. **Уровень бака:** три нержавеющих контактных болта в баке; нижний контакт — общий GND, средний и верхний — входы ESP32 с `INPUT_PULLUP`. Вода замыкает общий контакт с уровневым контактом → вход ON.
...
**Рекомендуемый полевой вариант:** линейный актуатор 12 V со встроенными концевиками, IP54/IP65, ход 100–300 mm и запас по усилию под фактическую форточку. Для управления нужен Н‑мост или пара реле реверса с электрической блокировкой направлений, предохранителем на 12 V и подавлением выбросов. Такой вариант ещё не реализован в `esphome/greenhouse-climate.yaml`; при переходе нужно заменить PCA9685/servo‑секции на дискретное управление OPEN/CLOSE и обновить cover‑логику.

**Текущий бюджетный/стендовый вариант:** PCA9685 + MG996R. Он оставлен в прошивке как дешёвый старт для лёгких створок, но не является лучшим выбором для тяжёлых или ответственных форточек во влажной теплице.
...
- Механика: серво → рычаг → форточка; установить **концевые упоры** и калибровать угол в HA (0° = закрыто, 90° = приоткрыто). Серво не должен держать вес створки без механического упора.
...
**Монтаж:** крепить концевик так, чтобы рычаг форточки **нажимал** переключатель только в крайнем положении. Проводка — в FTP‑кабеле рядом с питанием привода, но сигнальные пары держать отдельно от силовых жил; отдельная пара на каждый концевик: сигнал + GND. Подтяжка — `INPUT_PULLUP` в ESPHome, `inverted: true` (как контактные уровни бака на ESP32 №1).
